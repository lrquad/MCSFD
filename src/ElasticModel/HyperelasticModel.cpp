...
#include "materials/TypeStVKMaterial.h"
#include "materials/TypeNeoHookeanMaterial.h"
//#include "materials/TypeStVKBoostMaterial.h"
#include "materials/TypeStVKMaterialCSFD.h"
#include "materials/TypeNeoHookeanMaterialCSFD.h"
...
void Lobo::HyperelasticModel::runXMLscript(pugi::xml_node &xml_node)
{
    ...
    if (xml_node.child("Material"))
    {
        ...
        {
            this->elastic_material =
                new TypeStVKMaterialCSFD<double>(tetmesh, 0, 500.0);
            materialtype = "StVKCSFD";
        }
        ...
        if (strcmp("NeoHookeanCSFD", material_node.attribute("type").as_string()) ==
            0)
        {
            this->elastic_material =
                new TypeNeoHookeanMaterialCSFD<double>(tetmesh, 0, 500.0);
            materialtype = "NeoHookeanCSFD";
        }
...
#pragma omp for
        for (int i = 0; i < num_ele; i++)
        {
            ...
            {
                //test
                this->getTetForceMatrix(i, internalforce_list[i],
...
void Lobo::HyperelasticModel::computeAccelerationIndices(SparseMatrixTopologyTYPE<double> *sparsetopology)
{
    int numElements = tetmesh->getNumElements();
                                              ...
                                              Eigen::VectorXd *internalforce)
{
    ...
    // {
    //     int offset = i%3;
    //     internalforce->data()[i] = -internalforce->data()[0*3+offset]-internalforce->data()[1*3+offset]-internalforce->data()[2*3+offset];
    // }
}

void Lobo::HyperelasticModel::getTetForceMatrix(int eleid, Eigen::VectorXd *internalforce, Eigen::MatrixXd *stiffness, int computationflags)
{
    Eigen::Matrix3d F, Ds, m_U, m_V, m_singularF, P;
