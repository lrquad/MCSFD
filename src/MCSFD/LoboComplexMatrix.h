...
#define LoboComplexMatrix3d LoboComplexMatrix3<LoboComplext, double>
#define LoboComplexMatrix3dd LoboComplexMatrix3<LoboComplexDualt, double>
...
template <class COMPLEX_TYPE, class TYPE>
void lobo::LoboComplexMatrixTYPE::setZero(int flags)
{
	if (flags == MCSFD_all)
		memset(data.data(), 0, sizeof(COMPLEX_TYPE) * row_ * col_);

	if (flags == MCSFD_image)
	{
		for (int i = 0; i < row_; i++)
		{
			for (int j = 0; j < col_; j++)
			{
				data[j * row_ + i].image_ *= (TYPE)0.0;
			}
		}
	}
}
...
template <class COMPLEX_TYPE, class TYPE>
inline void multiMT(const LoboComplexMatrixTYPE3 &lhs, const LoboComplexMatrixTYPE3 &rhs, LoboComplexMatrixTYPE3 &result, int flag)
{
	//LoboComplexMatrix<COMPLEX_TYPE, TYPE> result(row, col);
	COMPLEX_TYPE tmp;
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			for (int k = 0; k < 3; k++)
			{
				lobo::multi<COMPLEX_TYPE, TYPE>(lhs.data[i * 3 + k], rhs.data[j * 3 + k], tmp, flag);
				//result.data[j*row + i] += lhs.data[k*lhs.row_ + i] * rhs.data[j*rhs.row_ + k];
...
template <class COMPLEX_TYPE, class TYPE>
inline void multiMScalar(const LoboComplexMatrixTYPE3 &lhs, const TYPE &rhs, LoboComplexMatrixTYPE3 &result, int flag)
{
	//LoboComplexMatrix<COMPLEX_TYPE, TYPE> result(row, col);
	//result.setZero();
	for (int i = 0; i < 9; i++)
	{
		...
		if (flag & MCSFD_image)
		{
			result.data[i].image_ = lhs.data[i].image_ * rhs;
		}
	}
}
...
inline double inner_productJ(const LoboComplexMatrix3d &a, const LoboComplexMatrix3d &b)
{
	double result = 0;
	for (int i = 0; i < 9; i++)
	{
		result += a.data[i].image_ * b.data[i].real_ + a.data[i].real_ * b.data[i].image_;
	}
	return result;
}

inline double inner_productH(const LoboComplexMatrix3dd &a, const LoboComplexMatrix3dd &b)
{
	double result = 0;
	for (int i = 0; i < 9; i++)
	{
		result += a.data[i].image_.image_ * b.data[i].real_.real_ + a.data[i].image_.real_ * b.data[i].real_.image_ + a.data[i].real_.image_ * b.data[i].image_.real_ + a.data[i].real_.real_ * b.data[i].image_.image_;
	}
	return result;
}

inline double traceSquareJ(const LoboComplexMatrix3d &a)
{
	double result_image = 0;
	double result_real = 0;
	for (int i = 0; i < 3; i++)
	{
		result_real += a.data[i * 3 + i].real_;
		result_image += a.data[i * 3 + i].image_;
	}
	return 2.0 * result_real * result_image;
}

inline double traceSquareH(const LoboComplexMatrix3dd &a)
{
	double result_image = 0;
	double result_real = 0;
	double result_real_image = 0;
	double result_image_real = 0;
	for (int i = 0; i < 3; i++)
	{
		result_real += a.data[i * 3 + i].real_.real_;
		result_image += a.data[i * 3 + i].image_.image_;
		result_image_real += a.data[i * 3 + i].image_.real_;
		result_real_image += a.data[i * 3 + i].real_.image_;
	}
	return 2.0 * result_real * result_image + 2.0 * result_image_real * result_real_image;
}

inline void  setMatrix3Value(int order, LoboComplexMatrix3dd &a, const double *value)
{
	for (int i = 0; i < 9; i++)
	{
		if (order == 0)
		{
			a.data[i].real_.real_ = value[i];
		}
		else if (order == 1)
		{
			/* code */
			a.data[i].real_.image_ = value[i];
		}
		else if (order == 2)
		{
			a.data[i].image_.real_ = value[i];
		}
		else if (order == 3)
		{
			a.data[i].image_.image_ = value[i];
		}
	}
}

inline void setMatrix3Value(LoboComplexMatrix3dd &a, const double *real_value, const LoboComplexMatrix3dd &b)
{
	//memcpy(a.data.data(),b.data.data(),sizeof(LoboComplexDualt)*9);
	for (int i = 0; i < 9; i++)
	{
		a.data[i].real_.real_ = real_value[i];
		a.data[i].real_.image_ = b.data[i].real_.image_;
		a.data[i].image_.real_ = b.data[i].image_.real_;
		a.data[i].image_.image_ = b.data[i].image_.image_;
	}
}

inline void getMatrix3Value(int order, double *value, const LoboComplexMatrix3dd &a)
{
	for (int i = 0; i < 9; i++)
	{
		if (order == 0)
		{
			value[i] = a.data[i].real_.real_;
		}
		else if (order == 1)
		{
			value[i] = a.data[i].real_.image_;
		}
		else if (order == 2)
		{
			value[i] = a.data[i].image_.real_;
		}
		else if (order == 3)
		{
			value[i] = a.data[i].image_.image_;
		}
	}
}

inline void setZerod(LoboComplexMatrix3dd &a,int flags)
{
	if (flags == MCSFD_all)
		memset(a.data.data(), 0, sizeof(LoboComplexDualt) * 3 * 3);
	...
	if (flags == MCSFD_image)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				a.data[j * 3 + i].image_.image_ = 0.0;
			}
		}
	}else if (flags == MCSFD_real_image)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				a.data[j * 3 + i].image_.real_ = 0.0;
				a.data[j * 3 + i].image_.image_ = 0.0;
			}
		}
	}	
...
inline void multiMTd(const LoboComplexMatrix3dd &lhs, const LoboComplexMatrix3dd &rhs, LoboComplexMatrix3dd &result, int flag)
{
	//LoboComplexMatrix<COMPLEX_TYPE, TYPE> result(row, col);
	LoboComplexDualt tmp;

	lobo::setZerod(result,flag);
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			for (int k = 0; k < 3; k++)
			{
				lobo::multi<LoboComplexDualt, double>(lhs.data[i * 3 + k], rhs.data[j * 3 + k], tmp, flag);
				//result.data[j*row + i] += lhs.data[k*lhs.row_ + i] * rhs.data[j*rhs.row_ + k];
				//lobo::add_a(result.data[j * 3 + i], tmp, flag);
				if (flag == MCSFD_all)
				{
					result.data[j * 3 + i].real_.real_ += tmp.real_.real_;
					result.data[j * 3 + i].image_.real_ += tmp.image_.real_;
					result.data[j * 3 + i].real_.image_ += tmp.real_.image_;
					result.data[j * 3 + i].image_.image_ += tmp.image_.image_;
				}
				else if (flag == MCSFD_image)
				{
					result.data[j * 3 + i].image_.image_ += tmp.image_.image_;
				}
				else if (flag == MCSFD_real_image)
				{
					result.data[j * 3 + i].image_.real_ += tmp.image_.real_;
...
inline void multiMScalard(const LoboComplexMatrix3dd &lhs, const double &rhs, LoboComplexMatrix3dd &result, int flag)
{
	//LoboComplexMatrix<COMPLEX_TYPE, TYPE> result(row, col);
	//result.setZero();
	for (int i = 0; i < 9; i++)
	{
		if (flag == MCSFD_all)
		{
			result.data[i].real_.real_ = lhs.data[i].real_.real_ * rhs;
			result.data[i].image_.real_ = lhs.data[i].image_.real_ * rhs;
			result.data[i].real_.image_ = lhs.data[i].real_.image_ * rhs;
			result.data[i].image_.image_ = lhs.data[i].image_.image_ * rhs;
		}
		else if (flag == MCSFD_image)
		{
			result.data[i].image_.image_ = lhs.data[i].image_.image_ * rhs;
...
void lobo::LoboComplexMatrix3<COMPLEX_TYPE, TYPE>::computeEigenVectors(LoboComplexMatrixTYPE3 &result, COMPLEX_TYPE *eigenvalues)
{
	...
	root2_d = sqrt3_ * term2dterm3 / ((TYPE)3.0 * CUBIC23) - nsqrt3_ * dual_term3_complex / ((TYPE)6.0 * CUBIC2);
	root2 = trace_ / (TYPE)3.0 + root2_d.real_;

	/*root2 = trace_ / (TYPE)3.0 + sqrt3*term2 / ((TYPE)3.0*CUBIC23*term3)
			- nsqrt3*term3 / ((TYPE)6.0*CUBIC2);*/

	root3_d = nsqrt3_ * term2dterm3 / ((TYPE)3.0 * CUBIC23) - sqrt3_ * dual_term3_complex / ((TYPE)6.0 * CUBIC2);
